return {
  {
    ...
    config = function()
      ...
      wk.add({
        ...
        { "<leader>dbl", function() dap.set_breakpoint(nil, nil, vim.fn.input('Log point message: ')) end, desc = "Log point" },
        ...
        { "<leader>of", group = "File" },
        { "<leader>ofb", ":Yazi<cr>", desc = "Browse File" },
        ...
        { "<leader>opf", telescope.git_files, desc = "File" },
        { "<leader>opa", telescope.find_files, desc = "All Files" },
      ...
      vim.api.nvim_create_autocmd("FileType", {
        ...
        callback = function()
          wk.add({
            ...
            { "<leader>lSs", ":VimtexStatus<cr>", icon = { icon = "", color = "blue" }, desc = "Show", buffer = true },
            { "<leader>lSa", ":VimtexStatusAll<cr>", icon = { icon = "", color = "blue" }, desc = "Show for all", buffer = true },

            { "<leader>li", group = 'Info', icon = { icon = "", color = "white" }, buffer = true },
            { "<leader>liL", ":VimtexImapsList<cr>", icon = { icon = "", color = "blue" }, desc = "Imaps List", buffer = true },
            { "<leader>lil", ":VimtexLog<cr>", icon = { icon = "", color = "yellow" }, desc = "Log", buffer = true },
            { "<leader>lie", ":VimtexErrors<cr>", icon = { icon = "", color = "red" }, desc = "Errors", buffer = true },
            { "<leader>lii", ":VimtexInfo<cr>", icon = { icon = "", color = "white" }, desc = "Info", buffer = true },
            { "<leader>lif", ":VimtexInfoFull<cr>", icon = { icon = "", color = "white" }, desc = "Full info", buffer = true },

            { "<leader>ls", group = 'Stop', icon = { icon = "󰜺", color = "red" }, buffer = true },
            { "<leader>lss", ":VimtexStop<cr>", icon = { icon = "", color = "red" }, desc = "Stop", buffer = true },
            { "<leader>lsa", ":VimtexStopAll<cr>", icon = { icon = "󰪧", color = "red" }, desc = "Stop all", buffer = true },

            { "<leader>lc", group = 'Compile', icon = { icon = "" }, buffer = true },
            { "<leader>lcc", ":VimtexClean<cr>", icon = { icon = "", color = "red" }, desc = "Clean", buffer = true },
            { "<leader>lcf", ":VimtexCleanFull<cr>", icon = { icon = "", color = "red" }, desc = "Full Clean", buffer = true },
            { "<leader>lcr", ":VimtexCompile<cr>", icon = { icon = "", color = "green" }, desc = "Run", buffer = true },
            { "<leader>lcs", ":VimtexCompileSelected<cr>", icon = { icon = "󰘦", color = "yellow" }, desc = "Selected", buffer = true },
            { "<leader>lco", ":VimtexcompileOutput<cr>", icon = { icon = "", color = "blue" }, desc = "Output", buffer = true },

            { "<leader>lt", group = 'Table of Content', icon = { icon = "", color = "orange" }, buffer = true },
            { "<leader>lto", ":VimtexTocOpen<cr>", icon = { icon = "", color = "orange" }, desc = "Open", buffer = true },
            { "<leader>ltt", ":VimtexTocToggle<cr>", icon = { icon = "󰨚", color = "orange" }, desc = "Toggle", buffer = true },

            { "<leader>lr", group = 'Reload', icon = { icon = "󰑓", color = "blue" }, buffer = true },
            { "<leader>lrr", ":VimtexReload<cr>", icon = { icon = "󰑓", color = "blue" }, desc = "Reload", buffer = true },
            { "<leader>lrs", ":VimtexReloadState<cr>", icon = { icon = "󱇯", color = "blue" }, desc = "Reload State", buffer = true },

            { "<leader>lv", ":VimtexView<cr>", icon = { icon = "", color = "green" }, desc = "View", buffer = true },
          })
        end
      })
      ...
      print("Debug: package.json detected=" .. vim.fn.filereadable(pkg_path))
      if root == "" then root = vim.fn.getcwd() end

      if vim.fn.filereadable(pkg_path) == 1 then
        wk.add({
          { "<leader>r", group = "Run", icon = { icon = "", color = "green" } },
          { "<leader>rs", ":lua require('telescope').extensions.nodescripts.run({})<CR>", desc = "Scripts", icon = { icon = "󰯁", color = "yellow" } },
      ...
      vim.api.nvim_create_autocmd("FileType", {
        ...
        callback = function()
          wk.add({
            ...
            { "<leader>cdcp", ":Dotnet new <CR>", desc = "Project", icon = { icon = "󰉗", color = "green" }, buffer = true },
            { "<leader>cdcf", ":Dotnet createfile <CR>", desc = "File", icon = { icon = "󰝒", color = "green" }, buffer = true },
            { "<leader>cdr", group = "Run", icon = { icon = "", color = "green" }, buffer = true },
            { "<leader>cdrr", ":Dotnet run<CR>", desc = "Run", icon = { icon = "", color = "green" }, buffer = true },
            { "<leader>cdrb", ":Dotnet build<CR>", desc = "Build", icon = { icon = "", color = "blue" }, buffer = true },
            { "<leader>cdrd", ":Dotnet debug<CR>", desc = "Debug", icon = { icon = "", color = "red" }, buffer = true },
            { "<leader>cdrt", ":Dotnet test<CR>", desc = "Test", icon = { icon = "󰙨", color = "yellow" }, buffer = true },
            { "<leader>cdl", group = "LSP", icon = { icon = "", color = "green" }, buffer = true },
            { "<leader>cdlS", ":Dotnet lsp stop<CR>", desc = "Stop", icon = { icon = "", color = "red" }, buffer = true },
            { "<leader>cdls", ":Dotnet lsp start<CR>", desc = "Start", icon = { icon = "", color = "green" }, buffer = true },
            { "<leader>cdlr", ":Dotnet lsp restart<CR>", desc = "Restart", icon = { icon = "", color = "blue" }, buffer = true },
          })
        end
      })
      ...
      local function merge_conflict_keys(bufnr)
        wk.add({
          ...
          { "<leader>mmh", ":diffget LOCAL<CR>", icon = { icon = "", color = "green" }, desc = "Left (Local)", buffer = bufnr },
          { "<leader>mml", ":diffget REMOTE<CR>", icon = { icon = "", color = "green" }, desc = "Right (Remote)", buffer = bufnr },
          { "<leader>mmb", ":diffget BASE<CR>", icon = { icon = "", color = "blue" }, desc = "Center (Base)", buffer = bufnr },

          { "<leader>mj", "]c", icon = { icon = "", color = "yellow" }, desc = "Next conflict", buffer = bufnr },
          { "<leader>mk", "[c", icon = { icon = "", color = "yellow" }, desc = "Previous conflict", buffer = bufnr },

          { "<leader>mq", ":cq<CR>", icon = { icon = "󰩈", color = "yellow" }, desc = "Exit merge tool", buffer = bufnr },

          { "<leader>md", group = "Diff", icon = { icon = "", color = "red" }, buffer = bufnr },
          { "<leader>mdu", ":diffupdate<CR>", icon = { icon = "󰚰", color = "blue" }, desc = "Update diff", buffer = bufnr },
          { "<leader>mdq", ":diffoff!<CR>", icon = { icon = "", color = "red" }, desc = "Toggle diff off", buffer = bufnr },
        })
      end
